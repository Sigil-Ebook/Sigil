    ...
    **Superceded by Base2 which is used for new seq handling class.**
    ...
    def _adddefaultproductions(self, productions, new=None):
        ...
        def COMMENT(expected, seq, token, tokenizer=None):
            ...
            seq.append(css_parser.css.CSSComment([token]))
            return expected
...
class Seq(object):
    ...
    def __str__(self):
        vals = []
        for v in self:
            if isinstance(v.value, string_type):
                vals.append(v.value)
...
class ListSeq(object):
    """
    (EXPERIMENTAL)
    A base class used for list classes like css_parser.css.SelectorList or
...
class _Namespaces(object):
    ...
    def items(self):
        return as_list(self.namespaces.items())

    def keys(self):
        return as_list(self.namespaces.keys())

    def values(self):
        return as_list(self.namespaces.values())
...
def _readUrl(url, fetcher=None, overrideEncoding=None, parentEncoding=None):
    ...
    ``fetcher``
        see css_parser.CSSParser.setFetcher for details
    ``overrideEncoding``
    ...
    --------------------------------
    **css_parser only**: 0. overrideEncoding
