...
class Profiles(object):
    """
    All profiles used for validation. ``css_parser.profile`` is a
    preset object of this class and used by all properties for validation.

    Predefined profiles are (use
    :meth:`~css_parser.profiles.Profiles.propertiesByProfile` to
    get a list of defined properties):

    :attr:`~css_parser.profiles.Profiles.CSS_LEVEL_2`
        Properties defined by CSS2.1
    :attr:`~css_parser.profiles.Profiles.CSS3_BASIC_USER_INTERFACE`
        Currently resize and outline properties only
    :attr:`~css_parser.profiles.Profiles.CSS3_BOX`
        Currently overflow related properties only
    :attr:`~css_parser.profiles.Profiles.CSS3_COLOR`
        CSS 3 color properties
    :attr:`~css_parser.profiles.Profiles.CSS3_PAGED_MEDIA`
        As defined at http://www.w3.org/TR/css3-page/ (at 090307)

    Predefined macros are:

    :attr:`~css_parser.profiles.Profiles._TOKEN_MACROS`
        Macros containing the token values as defined to CSS2
    :attr:`~css_parser.profiles.Profiles._MACROS`
    ...
    def _expand_macros(self, dictionary, macros):
        ...
        for key, value in as_list(dictionary.items()):
    ...
    def _compile_regexes(self, dictionary):
        ...
        for key, value in as_list(dictionary.items()):
            if not hasattr(value, '__call__'):
                # Compiling them now will slow down the css_parser import time,
                # even if css_parser is not needed. We lazily compile them the
                # first time they're needed.
                # https://bitbucket.org/cthedot/css_parser/issues/72
    ...
    def __update_knownNames(self):
        self._knownNames = []
        for properties in as_list(self._profilesProperties.values()):
            self._knownNames.extend(as_list(properties.keys()))
    ...
    def _setDefaultProfiles(self, profiles):
        ...
        # under python 2.X this was originally basestring but given unicode literals ...
        if isinstance(profiles, string_type):
    ...
    def addProfile(self, profile, properties, macros=None):
        ...
        :param properties:
            ...
            are reported or raised as all other css_parser exceptions depending
            on css_parser.log.raiseExceptions which e.g during parsing normally
        ...
        if macros:
            ...
            if len(set(macros.keys()).intersection(as_list(self._usedMacros.keys()))):
    ...
    def removeProfile(self, profile=None, all=False):
        ...
        :exceptions:
            - :exc:`css_parser.profiles.NoSuchProfileException`:
    ...
    def propertiesByProfile(self, profiles=None):
        ...
        elif isinstance(profiles, string_type):
            profiles = (profiles, )
        try:
    ...
    def validateWithProfile(self, name, value, profiles=None):
        ...
        Example::
            ...
            >>> print css_parser.profile.validateWithProfile('color', 'rgba(1,1,1,1)')
        ...
        if name not in self.knownNames:
            ...
            names = []
            for profilename, properties in as_list(self._profilesProperties.items()):
                # return profile to which name belongs
                if name in as_list(properties.keys()):
...
properties[Profiles.CSS_LEVEL_2] = {
    ...
    'display': r'inline|block|list-item|run-in|inline-block|table|inline-table|table-row-group|table-header-group|table-barter-group|table-row|table-column-group|table-column|table-cell|table-caption|none|inherit',  # noqa
