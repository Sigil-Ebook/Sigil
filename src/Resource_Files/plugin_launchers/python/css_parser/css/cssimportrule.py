...
class CSSImportRule(cssrule.CSSRule):
    ...
    def __init__(self, href=None, mediaText=None, name=None,
        ...
        if mediaText:
            ...
            self.media = css_parser.stylesheets.MediaList(mediaText='all')
        # 2. name
        self.name = name
    ...
    def __repr__(self):
        if self._usemedia:
            ...
            mediaText = None
        return "css_parser.css.%s(href=%r, mediaText=%r, name=%r)" % (
    ...
    def __str__(self):
        if self._usemedia:
            ...
            mediaText = None
        return "<css_parser.css.%s object href=%r mediaText=%r name=%r at 0x%x>"\
               % (self.__class__.__name__,
                  self.href,
                  mediaText,
                  self.name,
    ...
    def _getCssText(self):
        ...
        return css_parser.ser.do_CSSImportRule(self)
        ...
        if self._type(attoken) != self._prods.IMPORT_SYM:
            ...
            def _ident(expected, seq, token, tokenizer=None):
                ...
                if expected.startswith('media'):
                    ...
                    newMedia = css_parser.stylesheets.MediaList(parentRule=self)
    ...
    def _setHref(self, href):
        ...
        importedSheet = css_parser.css.CSSStyleSheet(media=self.media, ownerRule=self, title=self.name)
    ...
    def _setMedia(self, media):
        ...
        if isinstance(media, string_type):
            self._media = css_parser.stylesheets.MediaList(mediaText=media, parentRule=self)
        else:
    ...
    def _setName(self, name=''):
        ...
        # Under Python 2.X this was basestring but given unicode literals ...
        if name is None or isinstance(name, string_type):
            # "" or '' handled as None
            if not name:
                name = None
