...
#endif
#if !defined(Q_OS_WIN32) && !defined(Q_OS_MAC)
    // prefer the env var runtime overridden 'share/sigil/hunspell_dictionaries/' location.
    if (!sigil_extra_root.isEmpty()) {
        paths.append(sigil_extra_root + "/hunspell_dictionaries/");
    } else {
        // else use the standard build time location vendored in 'share/sigil/hunspell_dictionaries/'
        // or defined via the SYSTEM_HUNSPELL_DICTS cmake var.
