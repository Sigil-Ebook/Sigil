...
// Generate a unique path segment ending in file name for this reosurce
QString Resource::SegmentID(const QString& longest_common_path) const
{
    // Important: The longest common path must always ends with a "/"
    // same as generated by Utility::longestCommonPath
    return m_FullFilePath.right(m_FullFilePath.length() - longest_common_path.length());
}
